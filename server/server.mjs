...
async function operationsOverview() {
  ...
  return buildOperationsOverview({
    generatedAt:Date.now(), version:"7.69.62", releaseIdentifier:process.env.OCG_RELEASE_ID,
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "GET" && path === "/api/ready") {
      ...
      return json(res, ok ? 200 : 503, { ok, version:"7.69.62", releaseChannel:"EXTERNAL_ALPHA_CANDIDATE", status:shuttingDown ? "SHUTTING_DOWN" : database && !database.ok ? database.status : "READY", persistenceBackend:PROFILE_STORAGE_BACKEND, database:database ? { reachable:database.database.reachable, migrations:database.migrations, schemaReady:database.schemaReady } : null, roomStorage:rooms.storageLabel, matchmakingStorage:matchmaking.storageLabel });
    }
...
server.listen(PORT, HOST, () => {
  ...
  console.log(`Office Card Game v7.69.62 server running at http://${displayHost}:${PORT}`);
