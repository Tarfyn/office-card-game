...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "GET" && path === "/api/playtest/analytics/export") {
      ...
      if (format === "cards-csv") return text(res, 200, playtestCardActivityCsv(aggregatePlaytestAnalytics(records, now).cards), "text/csv; charset=utf-8", { "content-disposition": 'attachment; filename="office-card-game-card-activity-v7.67.csv"' });
...
server.listen(PORT, HOST, () => {
  ...
  console.log(`Office Card Game v7.67 server running at http://${displayHost}:${PORT}`);
