...
import { prepareLegacyGuestImport } from "../dist/src/guest-state.js";
...
const PRESERVED_PROFILE_MUTATION_ERRORS = new Set([
  ...
  "PLAYER_NOT_FOUND", "RANKED_SETTLEMENT_INCONSISTENT", "STARTER_AVATAR_INVALID", "STARTER_AVATAR_REQUIRED", "STARTER_AVATAR_ALREADY_CHOSEN", "STARTER_AVATAR_CHOICE_UNAVAILABLE", "STARTER_DEPARTMENT_INVALID", "STARTER_GRANT_EMPTY_POOL", "STARTER_ONBOARDING_COMPLETE", "STARTER_ONBOARDING_IN_PROGRESS", "STARTER_ONBOARDING_INVALID_STEP", "STARTER_ONBOARDING_NOT_STARTED"
]);
...
async function operationsOverview() {
  ...
  return buildOperationsOverview({
    generatedAt:Date.now(), version:"7.69.66", releaseIdentifier:process.env.OCG_RELEASE_ID,
...
function publicCatalog() {
  return Object.values(alphaDefinitions).map((card) => ({
    ...
    intentionalVanilla: card.intentionalVanilla === true,
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "GET" && path === "/api/ready") {
      ...
      return json(res, ok ? 200 : 503, { ok, version:"7.69.66", releaseChannel:"EXTERNAL_ALPHA_CANDIDATE", status:shuttingDown ? "SHUTTING_DOWN" : database && !database.ok ? database.status : "READY", persistenceBackend:PROFILE_STORAGE_BACKEND, database:database ? { reachable:database.database.reachable, migrations:database.migrations, schemaReady:database.schemaReady } : null, roomStorage:rooms.storageLabel, matchmakingStorage:matchmaking.storageLabel });
    }
    ...
    if (req.method === "POST" && path === "/api/onboarding/avatar") {
      const body = await readJson(req);
      const result = await mutateProfileForRequest(req, body?.profileToken, (service, token) => ({ profile:service.selectStarterAvatar(token, String(body?.avatarId ?? "")) }));
    ...
    if (req.method === "POST" && path === "/api/profiles/guest") {
      const body = await readJson(req);
      const imported = prepareLegacyGuestImport(body?.importMeta);
      // v7.69 sandbox snapshots are not production ownership state. New
      // guest profiles may retain a browser snapshot for compatibility, but
      // the explicit marker records that it is partial and Guest-only. It is
      // never consulted by the authenticated PostgreSQL Account path.
      const initialMeta = imported;
    ...
    if (req.method === "POST" && path === "/api/economy/booster/open") {
      ...
      const response = await mutateMetaForRequest(req, body, (context) => {
        const opened = openSandboxBooster(context.meta, Object.values(alphaDefinitions), {
          ...
          sourceRef:`booster:v1:${Number(context.meta.progression?.boostersOpened ?? 0) + 1}`,
    ...
    if (req.method === "POST" && path === "/api/rooms/bot") {
      ...
      const qaSetup = mode === "TUTORIAL"
    ? { fixedSeed:76960, fixedFirstPlayerId:"P2", forcePlayerOpeningFieldDefinitionIds:["N-001", "CS-021"], forceOpeningDefinitionIds:["N-002", "N-009", "N-005", "N-010", "N-013"], forceOpponentOpeningFieldDefinitionIds:["N-001"], initialCapacity:3 }
        : ALPHA_QA_EXECUTIVE_MATCH ? { forceOpeningHandVariantId: executiveEditionVariantId("CS-001") } : undefined;
      const tutorialDeckSelection = mode === "TUTORIAL" ? "it-starter" : deckSelection;
      const result = rooms.createBotRoom(tutorialDeckSelection, { mode }, profileIdentityForProfile(profile), botDeck, mode === "TUTORIAL" ? "Office Coach" : "Training Bot", qaSetup);
      return json(res, 201, result);
    }
...
server.listen(PORT, HOST, () => {
  ...
  console.log(`Office Card Game v7.69.66 server running at http://${displayHost}:${PORT}`);
