...
The unit keeps `NoNewPrivileges=true`, `RestrictSUIDSGID=true`, and the filesystem/device hardening
in place. It grants only ambient `CAP_SETUID`: the fixed root-owned helper intentionally drops to
the `postgres` OS user for `pg_dump` and `pg_restore`, and the hardened systemd context otherwise
removes that capability from the effective/permitted sets. No `CAP_SETGID` or broader ambient
