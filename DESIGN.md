...
Outcome dwell is separate from that 320 ms blocking combat envelope. A single event-owned result host remains for 500 ms (winner/loser), 560 ms (both-archived draw) or 550 ms (nonlethal direct REP). Archive starts on schedule; departing faces hand off to their existing proxies while the result frame and stamp remain readable. A 600 ms V1 Archive/rejection mark overlaps travel. The lethal result host uses only the existing impact-to-result window (440 ms for a complete direct lethal); no gate or post-result-screen dwell is added. All existing motion, impact, easing, queue budget and transport tokens remain unchanged. See [outcome dwell](docs/vfx-outcome-dwell.md).

The persistence pass retains those recognition floors and gives the same host a non-blocking tail: 800 ms total winner/loser and REP, 900 ms DRAW and Archive/rejection marks. Lethal impact layers last 1000 ms while the critical panel still clears at 440 ms. The finite signature stays below the opaque result card; the surrounding backdrop temporarily omits blur so its decay remains visible. No result or queue wait is added. Targeting/catch-up may retire tails after their critical floor, and reset/reconciliation removes them. Executive and other ENGINE signatures remain 850 ms. Test-only timestamp instrumentation separates transport, server processing, render/paint and semantic queue contention; it is absent from normal client/server execution. See [persistence and latency](docs/vfx-persistence-latency.md).
