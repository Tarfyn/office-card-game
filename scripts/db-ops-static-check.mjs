...
const deployScript = read("ops/office-card-game-deploy.sh");
...
for (const [name, content] of [
  ...
  ["ops/office-card-game-deploy.sh", deployScript],
...
if (hasBash) {
  for (const relative of ["ops/ocg-db-helper", "ops/install-db-helper.sh", "ops/office-card-game-deploy.sh"]) {
...
assert.match(deployScript, /marker_size=.*stat --format='%s' "\$CUTOVER_MARKER"/);
assert.match(deployScript, /marker_value.*< "\$CUTOVER_MARKER"/);
...
assert.match(deployScript, /die "PostgreSQL migration failed; the previous release remains active\."/);
