...
if (missing.length || extra.length || incomplete.length) {
  console.error(JSON.stringify({ missing, extra, incomplete }, null, 2));
  process.exit(1);
}
console.log(`I18N_AUDIT_OK · de ${translatedIds.size}/${cards.length} cards · names/rules/flavor covered`);
