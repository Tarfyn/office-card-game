...
let passed=0;const root=(n:string)=>readFileSync(fileURLToPath(new URL(`../../${n}`,import.meta.url)),"utf8");const pkg=JSON.parse(root("package.json")),server=root("server/server.mjs"),html=root("public/index.html"),script=root("scripts/artwork-audit.mjs"),docs=root("ARTWORK_PIPELINE.md");function test(n:string,f:()=>void){f();passed++;console.log(`✓ ${n}`)}
