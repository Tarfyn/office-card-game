...
test("v2.8 authoritative room validation rejects illegal custom decks", () => {
  ...
  assert.throws(() => rooms.createRoom({ id:"custom:bad", name:"Illegal", cards:[{definitionId:"CS-001",copies:4}] }), (error:any) => error instanceof RoomError && error.code === "INVALID_DECK");
});
