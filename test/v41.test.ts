...
test("v4.1 telemetry persists across restart and does not charge server downtime to decision time", () => {
  ...
  let tokenNo = 0;
  const base = { tokenFactory:()=>`per-${++tokenNo}`, seedFactory:()=>43, firstPlayerFactory:()=>"P1" as const, nowFactory:()=>now.value, persistence:store.adapter };
