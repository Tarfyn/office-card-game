...
test("German card overlay covers exactly the 107-card Alpha pool",()=>{assert.equal(cards.length,107);for(const card of cards)assert.match(deCards,new RegExp(`\\"${card.id}\\"`))});
