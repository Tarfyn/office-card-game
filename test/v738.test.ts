...
test("v7.38 expands the Alpha pool from 97 to exactly 107 definitions",()=>{
  ...
  for(const id of newIds) assert.ok(alphaDefinitions[id],`${id} missing`);
  const expected:any={CUSTOMER_SERVICE:22,IT:20,OFFICE:17,MARKETING:17,PRODUCTION:17,NEUTRAL:14};
  for(const [department,count] of Object.entries(expected)) assert.equal(Object.values(alphaDefinitions).filter(c=>c.department===department).length,count);
});
...
test("v7.38 Expansion I cards are full, flavored and use existing card/effect vocabulary",()=>{
  ...
  for(const id of newIds){const card=alphaDefinitions[id];assert.equal(card.implementationStatus,"FULL");assert.ok(card.flavorText);for(const ability of card.abilities??[])for(const effect of (ability as any).effects??[]){assert.ok(allowedEffects.has(effect.type),`${id} introduced ${effect.type}`);if(effect.type==="IF")for(const nested of effect.then)assert.ok(allowedEffects.has(nested.type),`${id} introduced nested ${nested.type}`)}}
});

test("v7.38 resolves the targeted early-body and reactive-depth audit gaps",()=>{
  const audit=analyzeContentGaps();
  for(const dep of ["CUSTOMER_SERVICE","OFFICE"])assert.equal(audit.gaps.some(g=>g.department===dep&&g.kind==="EARLY_EMPLOYEE_VARIETY"),false);
  for(const dep of ["MARKETING","PRODUCTION"])assert.equal(audit.gaps.some(g=>g.department===dep&&g.kind==="REACTIVE_DEPTH"),false);
...
test("v7.38 leaves all five tuned starters unchanged and legal at 40 cards",()=>{
  for(const deck of Object.values(alphaDeckPresets)){assert.equal(deck.cards.reduce((sum,e)=>sum+e.copies,0),40);assert.equal(deck.cards.some(e=>newIds.includes(e.definitionId)),false)}
});
...
test("v7.38 ships the documented ten-card Expansion I contract",()=>{
  ...
  assert.match(doc,/97 → 107/);assert.match(doc,/No starter deck changes/i);assert.match(doc,/No new effect type/i);
  for(const id of newIds) assert.match(doc,new RegExp(id));
});
...
test("v7.38 Triage Agent and Warm Transfer execute through existing Power rules",()=>{
  ...
  mulligan(state,"P1",[]);mulligan(state,"P2",[]);advancePhase(state,"P1");advancePhase(state,"P1");
  const agent=findInHandByDefinition(state,"P1","CS-021")!;const transfer=findInHandByDefinition(state,"P1","CS-022")!;
  playEmployee(state,"P1",agent,0);playAction(state,"P1",transfer,{TARGET_1:[agent]});
...
test("v7.38 Shared Inbox uses the existing first-card cost modifier pipeline",()=>{
  ...
  mulligan(state,"P1",[]);mulligan(state,"P2",[]);advancePhase(state,"P1");advancePhase(state,"P1");
  const inbox=findInHandByDefinition(state,"P1","N-015")!;const email=findInHandByDefinition(state,"P1","MKT-011")!;
  playSystem(state,"P1",inbox,0);const cost=getCardCost(state,"P1",email,"PLAY");assert.equal(cost.printedCost,2);assert.equal(cost.finalCost,1);
});
