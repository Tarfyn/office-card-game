import { strict as assert } from "node:assert";import { readFileSync } from "node:fs";import { fileURLToPath } from "node:url";import { analyzeContentGaps } from "../src/content-audit.js";
...
test("v7.37 identifies current shallow 15-card pools",()=>{for(const dep of ["OFFICE","MARKETING","PRODUCTION"])assert.ok(audit.gaps.some(g=>g.department===dep&&g.kind==="POOL_DEPTH"))});
