...
function stores(initialPlayers:PlayerDataStoreSnapshot|null=null) {
  let players = initialPlayers ? structuredClone(initialPlayers) : null;
  let credentials:GuestCredentialStoreSnapshot|null = null;
  return {
    playerPersistence:{ storageLabel:"PLAYERS", load:()=>players ? structuredClone(players) : null, save:(snapshot:PlayerDataStoreSnapshot)=>{ players=structuredClone(snapshot); } },
    credentialPersistence:{ storageLabel:"CREDS", load:()=>credentials ? structuredClone(credentials) : null, save:(snapshot:GuestCredentialStoreSnapshot)=>{ credentials=structuredClone(snapshot); } },
    seedCredential:(profileToken:string, playerId:string)=>{ credentials={version:1,credentials:[{kind:"GUEST_LOCAL",profileToken,playerId,createdAt:1,lastUsedAt:1}]}; },
    readPlayers:()=>players
  };
}
...
test("v4.7 five placement matches promote a profile to rated and resign losses stay separate", () => {
  ...
  const a=service.create(); const b=service.create();
  for (let i=1;i<=5;i++) service.recordRankedMatch({roomId:`R${i}`,p1PlayerId:a.profile.playerId,p2PlayerId:b.profile.playerId,winnerPlayerId:i===5?b.profile.playerId:a.profile.playerId,reason:i===5?"RESIGN":"REPUTATION_ZERO"});
...
test("v4.7 existing v4.6 player snapshots gain ranked defaults without losing player data", () => {
  ...
  const legacyProfile:any = structuredClone(created.profile);
...
test("v4.7 ranked matchmaking prefers close MMR and widens while players wait", () => {
  let now=0, seq=0;
  const queue = new MatchmakingQueue<any,any>({
...
test("v4.7 server/client wire preseason rating while Ranked timer remains disabled", () => {
  ...
  assert.equal(settings.timerProfiles.find((p:any)=>p.id==="RANKED_STANDARD_TBD").enabled,false);
