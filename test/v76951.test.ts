...
assert.equal(packageJson.version, "7.69.62");
assert.equal(packageLock.version, "7.69.62");
assert.equal(packageLock.packages[""].version, "7.69.62");
assert.match(server, /version: "7\.69\.62"/);
assert.match(server, /version:"7\.69\.62"/);
...
console.log("v7.69.62 Starter Intern release-gate markers passed.");
