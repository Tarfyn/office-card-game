...
assert.equal(packageJson.version, "7.69.69");
assert.equal(packageLock.version, "7.69.69");
assert.equal(packageLock.packages[""].version, "7.69.69");
assert.match(server, /version: "7\.69\.69"/);
assert.match(server, /version:"7\.69\.69"/);
...
console.log("v7.69.69 Executive Edition variant ownership release-gate markers passed.");
