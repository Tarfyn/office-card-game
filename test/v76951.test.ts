...
assert.equal(packageJson.version, "7.69.56");
assert.equal(packageLock.version, "7.69.56");
assert.equal(packageLock.packages[""].version, "7.69.56");
assert.match(server, /version: "7\.69\.56"/);
assert.match(server, /version:"7\.69\.56"/);
...
console.log("v7.69.56 Starter Onboarding release-gate markers passed.");
