...
assert.equal(packageJson.version, "7.69.73");
assert.equal(packageLock.version, "7.69.73");
assert.equal(packageLock.packages[""].version, "7.69.73");
assert.match(server, /version: "7\.69\.73"/);
assert.match(server, /version:"7\.69\.73"/);
...
console.log("v7.69.73 Executive Edition variant ownership release-gate markers passed.");
