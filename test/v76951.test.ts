...
assert.equal(packageJson.version, "7.69.70");
assert.equal(packageLock.version, "7.69.70");
assert.equal(packageLock.packages[""].version, "7.69.70");
assert.match(server, /version: "7\.69\.70"/);
assert.match(server, /version:"7\.69\.70"/);
...
console.log("v7.69.70 Executive Edition variant ownership release-gate markers passed.");
