...
assert.equal(packageJson.version, "7.69.65");
assert.equal(packageLock.version, "7.69.65");
assert.equal(packageLock.packages[""].version, "7.69.65");
assert.match(server, /version: "7\.69\.65"/);
assert.match(server, /version:"7\.69\.65"/);
...
console.log("v7.69.65 Executive Edition variant ownership release-gate markers passed.");
