...
assert.equal(packageJson.version, "7.69.59");
assert.equal(packageLock.version, "7.69.59");
assert.equal(packageLock.packages[""].version, "7.69.59");
assert.match(server, /version: "7\.69\.59"/);
assert.match(server, /version:"7\.69\.59"/);
...
console.log("v7.69.59 Starter Intern release-gate markers passed.");
