...
assert.equal(packageJson.version, "7.69.64");
assert.equal(packageLock.version, "7.69.64");
assert.equal(packageLock.packages[""].version, "7.69.64");
assert.match(server, /version: "7\.69\.64"/);
assert.match(server, /version:"7\.69\.64"/);
...
console.log("v7.69.64 Starter Intern release-gate markers passed.");
