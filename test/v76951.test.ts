...
assert.equal(packageJson.version, "7.69.74");
assert.equal(packageLock.version, "7.69.74");
assert.equal(packageLock.packages[""].version, "7.69.74");
assert.match(server, /version: "7\.69\.74"/);
assert.match(server, /version:"7\.69\.74"/);
...
console.log("v7.69.74 Executive Edition variant ownership release-gate markers passed.");
