...
assert.equal(packageJson.version, "7.69.60");
assert.equal(packageLock.version, "7.69.60");
assert.equal(packageLock.packages[""].version, "7.69.60");
assert.match(server, /version: "7\.69\.60"/);
assert.match(server, /version:"7\.69\.60"/);
...
console.log("v7.69.60 Starter Intern release-gate markers passed.");
