...
function match(p1: Array<{definitionId:string;copies:number}>, p2: Array<{definitionId:string;copies:number}>): GameState {
  const state = createMatch({ matchId:"v13", seed:1300, firstPlayerId:"P1", definitions:alphaDefinitions, p1Deck:p1, p2Deck:p2 });
  mulligan(state,"P1",[]); mulligan(state,"P2",[]); return state;
}
...
function forceToHand(state: GameState, playerId: PlayerId, definitionId: string): string {
  const p=state.players[playerId]; const existing=p.hand.find(id=>state.cards[id].definitionId===definitionId); if(existing) return existing;
  const idx=p.deck.findIndex(id=>state.cards[id].definitionId===definitionId); if(idx<0) throw new Error(`No ${definitionId}`);
  const [id]=p.deck.splice(idx,1); state.cards[id].zone="HAND"; state.cards[id].objectVersion+=1; p.hand.push(id); return id;
}
function forceEmployee(state: GameState, playerId: PlayerId, definitionId: string, slot: number): string {
  const id=forceToHand(state,playerId,definitionId); state.players[playerId].hand=state.players[playerId].hand.filter(x=>x!==id);
  const c=state.cards[id]; c.zone="EMPLOYEE_FIELD"; c.slot=slot; c.faceUp=true; c.onboarding=false; c.controllerId=playerId; c.enteredFieldTurnNumber=0; state.players[playerId].employeeField[slot]=id; return id;
}
function resolveCurrentTrigger(state: GameState, playerId: PlayerId, targets: Record<string,string[]>) {
  const sel=state.pendingTriggerTargetSelection; assert(sel && sel.playerId===playerId,"Expected trigger target selection");
  resolveTriggerTargetSelection(state,playerId,sel.id,targets);
  if (state.responseWindow) { const first=state.priorityPlayerId!; passPriority(state,first); if(state.responseWindow) passPriority(state,state.priorityPlayerId!); }
}
...
test("Service Desk Lead pauses for a player-selected opposing target",()=>{
  ...
  toMain(state,"P1"); state.players.P1.availableCapacity=30;
  const mat=forceToHand(state,"P1","IT-001"); playEmployee(state,"P1",mat,0);
  const lead=forceToHand(state,"P1","IT-002"); playEmployee(state,"P1",lead,0,[mat]);
  const enemy=forceEmployee(state,"P2","CS-001",0);
  const ticket=forceToHand(state,"P1","IT-001"); playEmployee(state,"P1",ticket,1);
...
test("Office Manager target selector excludes itself",()=>{
  ...
  toMain(state,"P1"); state.players.P1.availableCapacity=30;
  const manager=forceToHand(state,"P1","OFC-003"); playEmployee(state,"P1",manager,0);
  ...
  const secondMeetingCard=forceToHand(state,"P1","OFC-003"); playEmployee(state,"P1",secondMeetingCard,2);
  const sel=state.pendingTriggerTargetSelection; assert(sel,"Expected Office Manager target selection");
...
test("Social Media Manager draws then pauses for one hand card to Archive",()=>{
  ...
  toMain(state,"P1"); state.players.P1.availableCapacity=40;
  const smm=forceToHand(state,"P1","MKT-001"); playEmployee(state,"P1",smm,0);
  const a1=forceToHand(state,"P1","MKT-009"); playAction(state,"P1",a1,{TARGET_1:[smm]});
  const beforeSecond=state.players.P1.hand.length;
  const a2=forceToHand(state,"P1","MKT-009"); playAction(state,"P1",a2,{TARGET_1:[smm]});
...
test("Follow-Up Email returns a Ticket Action but prevents replay that turn",()=>{
  ...
  toMain(state,"P1"); state.players.P1.availableCapacity=30;
  const ticket=forceToHand(state,"P1","CS-008"); state.players.P1.hand=state.players.P1.hand.filter(x=>x!==ticket); state.cards[ticket].zone="ARCHIVE"; state.players.P1.archive.push(ticket);
  const follow=forceToHand(state,"P1","CS-017"); playAction(state,"P1",follow,{TARGET_1:[ticket]});
  assert((state.cards[ticket].zone as string)==="HAND","Ticket should return to hand");
...
test("Ticket Closed shuffles a Ticket Action into Deck and draws one",()=>{
  ...
  toMain(state,"P1"); state.players.P1.availableCapacity=30;
  const target=forceToHand(state,"P1","CS-008"); state.players.P1.hand=state.players.P1.hand.filter(x=>x!==target); state.cards[target].zone="ARCHIVE"; state.players.P1.archive.push(target);
  const tc=forceToHand(state,"P1","IT-009"); const before=state.players.P1.hand.length; playAction(state,"P1",tc,{TARGET_1:[target]});
  assert((state.cards[target].zone as string)==="DECK","Target Ticket Action should be in Deck");
...
test("Staging Environment plays a cheap IT System through Deploy for one less",()=>{
  ...
  toMain(state,"P1"); state.players.P1.availableCapacity=20;
  const staging=forceToHand(state,"P1","IT-020"); playSystem(state,"P1",staging,0);
...
test("Escalated Ticket can count as two Promotion materials",()=>{
  ...
  toMain(state,"P1"); state.players.P1.availableCapacity=40;
  const worker=forceToHand(state,"P1","PRD-001"); state.players.P1.hand=state.players.P1.hand.filter(x=>x!==worker); state.cards[worker].zone="ARCHIVE"; state.players.P1.archive.push(worker);
  const escalated=forceToHand(state,"P1","CS-009"); playAction(state,"P1",escalated,{TARGET_1:[worker]});
  assert((state.cards[worker].zone as string)==="EMPLOYEE_FIELD","Worker should return to field");
...
test("Packaging Machine excludes the battle attacker from its target candidates",()=>{
  ...
  toMain(state,"P1"); state.players.P1.availableCapacity=30;
  const machine=forceToHand(state,"P1","PRD-015"); playSystem(state,"P1",machine,0);
  const attacker=forceEmployee(state,"P1","PRD-001",0); const other=forceEmployee(state,"P1","PRD-001",1); const defender=forceEmployee(state,"P2","IT-001",0);
  ...
  const sel=state.pendingTriggerTargetSelection; assert(sel,"Packaging Machine should request a target");
  const candidates=sel.targetChoices[0].candidateIds;
...
test("trigger target selection is exposed through legal actions and blocks other commands",()=>{
  ...
  toMain(state,"P1"); state.players.P1.availableCapacity=30;
  const mat=forceToHand(state,"P1","IT-001"); playEmployee(state,"P1",mat,0);
  const lead=forceToHand(state,"P1","IT-002"); playEmployee(state,"P1",lead,0,[mat]); forceEmployee(state,"P2","CS-001",0);
  const ticket=forceToHand(state,"P1","IT-001"); playEmployee(state,"P1",ticket,1);
...
test("Customer Satisfaction Survey lets its controller choose the Employee receiving the Review buff",()=>{
  ...
  toMain(state,"P1"); state.players.P1.availableCapacity=30; state.players.P1.reputation=10;
  const survey=forceToHand(state,"P1","CS-020"); playSystem(state,"P1",survey,0);
  const agent=forceToHand(state,"P1","CS-001"); playEmployee(state,"P1",agent,0);
  const review=forceToHand(state,"P1","CS-014"); playAction(state,"P1",review);
...
test("Operations Director draws after an opponent Action is delayed",()=>{
  ...
  const approval=forceToHand(state,"P1","OFC-007"); setIncident(state,"P1",approval,0);
  advancePhase(state,"P1"); advancePhase(state,"P1"); advancePhase(state,"P1");
  toMain(state,"P2"); state.players.P2.availableCapacity=30;
  const viral=forceToHand(state,"P2","MKT-012"); const before=state.players.P1.hand.length;
  playActionInteractive(state,"P2",viral);
  activateResponse(state,"P1",approval,"OFC-007-A1");
  passPriority(state,"P2"); passPriority(state,"P1");
  ...
  while(state.responseWindow && state.priorityPlayerId){ passPriority(state,state.priorityPlayerId); }
...
test("Campaign Dashboard turns the first Content Action into a top-two Campaign selection",()=>{
  ...
  toMain(state,"P1"); state.players.P1.availableCapacity=30;
  const dashboard=forceToHand(state,"P1","MKT-015"); playSystem(state,"P1",dashboard,0);
  ...
  const campaign=forceToHand(state,"P1","MKT-012"); state.players.P1.hand=state.players.P1.hand.filter(id=>id!==campaign); state.cards[campaign].zone="DECK"; state.players.P1.deck.push(campaign);
...
let passed=0;
for(const [name,fn] of tests){ try{ fn(); passed++; console.log(`✓ ${name}`); }catch(error){ console.error(`✗ ${name}`); throw error; } }
