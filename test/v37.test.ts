...
test("v3.7 profile persistence restores the same secret-token profile after service restart", () => {
  let persisted: PlayerProfileStoreSnapshot | null = null;
  const persistence = {
    storageLabel:"TEST_PERSISTENT",
    load:() => persisted ? structuredClone(persisted) : null,
    save:(snapshot:PlayerProfileStoreSnapshot) => { persisted = structuredClone(snapshot); }
  };
...
test("v3.7 match history updates persistent W-L stats once per room", () => {
  ...
  const entry = { roomId:"ROOM01", matchId:"match-ROOM01", mode:"FRIENDLY" as const, outcome:"WIN" as const, opponentName:"Opponent", deckName:"IT Starter", opponentDeckName:"Office Starter", turns:9, reason:"REPUTATION_ZERO", finishedAt:500 };
