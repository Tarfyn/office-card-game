...
test("v3.8 active match and processed intent cache survive RoomService restart", () => {
  ...
  const hostMulligan = first.submitIntent(host.roomId, host.token, { intentId:"host-keep", expectedStateVersion:hostView.match!.stateVersion, intent:{ type:"MULLIGAN" as const, returnIds:[] } });
  assert.equal(hostMulligan.response.accepted, true);
  const guestView = first.getView(host.roomId, guest.token, 0);
  const guestMulligan = first.submitIntent(host.roomId, guest.token, { intentId:"guest-keep", expectedStateVersion:guestView.match!.stateVersion, intent:{ type:"MULLIGAN" as const, returnIds:[] } });
  ...
  const request = { intentId:"persisted-resign", expectedStateVersion:hostView.match!.stateVersion, intent:{ type:"RESIGN" as const } };
...
test("v3.8 waiting Quick Match ticket survives queue restart and still matches", () => {
  ...
  let persisted: MatchmakingStoreSnapshot<Payload, Session> | null = null;
  const persistence = {
    storageLabel:"TEST_MM",
    load:() => persisted ? structuredClone(persisted) : null,
    save:(snapshot:MatchmakingStoreSnapshot<Payload, Session>) => { persisted = structuredClone(snapshot); }
  };
  let n = 0;
  const first = new MatchmakingQueue<Payload, Session>({ ticketIdFactory:()=>`m${++n}`, nowFactory:()=>n, persistence });
  const a = first.enqueue("profile-a", "FRIENDLY", {deck:"A"});
  assert.equal(a.opponent, null);
  const restarted = new MatchmakingQueue<Payload, Session>({ ticketIdFactory:()=>`m${++n}`, nowFactory:()=>n, persistence });
...
test("v3.8 matched Quick Match sessions persist as one pair", () => {
  ...
  let n = 0;
  const queue = new MatchmakingQueue<Payload, Session>({ ticketIdFactory:()=>`p${++n}`, nowFactory:()=>n, persistence });
  ...
  const restarted = new MatchmakingQueue<Payload, Session>({ persistence });
