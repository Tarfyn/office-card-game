...
test("v7.69.17 version markers are current",()=>{
  assert.equal(pkg.version,"7.69.28");
  assert.match(server,/version: "7\.69\.28"/);
  assert.match(server,/version:"7\.69\.28"/);
  assert.match(server,/Office Card Game v7\.69\.28 server/);
...
test("Lobby showcase reuses the Deckbuilder catalog card face",()=>{
  ...
  assert.match(helper,/renderCatalogCardFace\(def,\{ artReady:Boolean\(def\.artId\) \}\)/);
