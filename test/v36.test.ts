...
test("v3.6 RoomService projects playtest names without exposing profile ids", () => {
  ...
  assert.equal((guest.view as unknown as Record<string, unknown>).profileId, undefined);
...
test("v3.6 matchmaking pairs oldest compatible queue entry and never self-matches", () => {
  let n = 0;
  const queue = new MatchmakingQueue<{deck:string},{room:string}>({ ticketIdFactory:()=>`t${++n}`, nowFactory:()=>n });
