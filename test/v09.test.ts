...
import type { CardDefinition, DeckEntry, GameState, MatchIntentCommand, PlayerId } from "../src/types.js";
...
function deck(definitionId: string): DeckEntry[] { return [{ definitionId, copies: 40 }]; }

function createBasic(definitions = alphaDefinitions, p1 = "CS-001", p2 = "IT-001"): GameState {
  return createMatch({
    matchId: "v09-test",
    seed: 909,
    firstPlayerId: "P1",
...
function readyMatch(definitions = alphaDefinitions, p1 = "CS-001", p2 = "IT-001"): GameState {
...
function toMain(state: GameState, playerId: PlayerId): void {
  while (state.activePlayerId === playerId && state.phase !== "MAIN") advancePhase(state, playerId);
...
function command(state: GameState, playerId: PlayerId, intent: MatchIntentCommand["intent"], intentId = "intent"): MatchIntentCommand {
...
test("targeted Actions project exact legal target candidates", () => {
  const definitions: Record<string, CardDefinition> = { ...alphaDefinitions };
...
test("face-down opponent Support targets use opaque client refs that resolve server-side", () => {
  const testAction: CardDefinition = {
...
test("Match Intent API increments stateVersion and rejects stale commands without mutation", () => {
  ...
  const stale: MatchIntentCommand = { intentId: "stale", matchId: state.matchId, playerId: "P2", expectedStateVersion: 0, intent: { type: "MULLIGAN", returnIds: [] } };
