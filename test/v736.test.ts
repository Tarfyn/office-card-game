import { strict as assert } from "node:assert";import { readFileSync } from "node:fs";import { fileURLToPath } from "node:url";import { runBalanceMatchupSet } from "../src/balance.js";
...
test("v7.36 tracks when-seen and when-played win rates separately",()=>{const row=r.cardStats.find(x=>x.gamesSeen>0);if(!row) throw new Error("missing card stat");assert.ok(row.winRateWhenSeen===null||typeof row.winRateWhenSeen==="number");assert.ok(row.winRateWhenPlayed===null||typeof row.winRateWhenPlayed==="number")});
test("v7.36 records copies played without mutating rules",()=>{for(const row of r.cardStats){assert.ok(row.totalCopiesPlayed>=row.gamesPlayed);assert.ok(row.averageCopiesPlayed>=0)}});
test("v7.36 card stats include identity metadata",()=>{const row=r.cardStats[0];if(!row) throw new Error("missing card stat");assert.ok(row.definitionId);assert.ok(row.name);assert.ok(row.department);assert.ok(row.cardType)});
