...
## v7.69.5 — Full-Desk Lobby Geometry, Artwork Rollout + Match HUD Fixes

v7.69.5 consolidates the next visual playtest pass into one deployable update. The Executive Desk now owns the full lobby viewport instead of appearing as a wooden object on a second wooden background; its open utility drawer is exactly flush with the desk edges, the drawer seam reads from above, and the brass pull sits on the lower drawer front to match the downward opening perspective. The 4K layout uses the available desk width while mobile compacts the Player File and keeps secondary utilities inside the same full-width drawer. Quick Match and the drawer receive a small material/readability refinement rather than another redesign.

The supplied WebP artwork batch is integrated under `public/art/alpha/` with canonical kebab-case card slugs. 81 supplied illustrations replace their corresponding card artwork references; together with the retained Approval Required illustration, 82 of 107 current cards now have production artwork and 25 remain missing. The artwork audit accepts the batch’s near-16:9 1376×768 source ratio while keeping 1600×900 as the preferred canonical canvas.

The same release also carries the two board-first presentation fixes held for this bundle: Office Coach now uses a compact one-column layout inside the narrow Match HUD rail, and direct attacks anchor the connector plus destination highlight to the visible defending REP vital instead of the hidden legacy Reputation detail card. Authoritative combat, board topology, matchmaking, hosted sync, economy, Ranked timer behavior and rematch semantics are unchanged.
