...
import { advancePhase, createMatch, declareAttack, getCurrentPower, mulligan, playAction, playEmployee, playSystem } from "../src/engine.js";
...
function forceToHand(state, playerId, definitionId) {
    ...
    if (index < 0)
        throw new Error(`No ${definitionId} available`);
...
test("Customer Service Agent triggers on the first later Ticket card and only once per turn", () => {
    const state = mixedMatch([{ definitionId: "CS-001", copies: 40 }], [{ definitionId: "IT-001", copies: 40 }]);
    toMain(state, "P1");
...
test("Call Center Agent gains Capacity exactly when the second Call card of the turn is played", () => {
    const state = mixedMatch([{ definitionId: "CS-002", copies: 20 }, { definitionId: "CS-001", copies: 20 }], [{ definitionId: "IT-001", copies: 40 }]);
    toMain(state, "P1");
    state.players.P1.availableCapacity = 20;
...
test("Team Lead Customer Service buffs other CS Employees only during the opponent turn", () => {
    const state = mixedMatch([{ definitionId: "CS-001", copies: 30 }, { definitionId: "CS-006", copies: 10 }], [{ definitionId: "IT-001", copies: 40 }]);
    toMain(state, "P1");
...
test("Review Portal adds one extra Reputation to the first restore each turn", () => {
    const state = mixedMatch([{ definitionId: "CS-013", copies: 10 }, { definitionId: "CS-014", copies: 30 }], [{ definitionId: "IT-001", copies: 40 }]);
    toMain(state, "P1");
    state.players.P1.availableCapacity = 20;
...
test("DevOps Engineer gains Capacity after a System is played through Deploy", () => {
    const state = mixedMatch([{ definitionId: "IT-004", copies: 10 }, { definitionId: "IT-011", copies: 10 }, { definitionId: "IT-014", copies: 20 }], [{ definitionId: "CS-001", copies: 40 }]);
    toMain(state, "P1");
    state.players.P1.availableCapacity = 20;
...
test("Calendar Block prevents its target from attacking during its next Battle Phase", () => {
    const state = mixedMatch([{ definitionId: "OFC-008", copies: 20 }, { definitionId: "CS-001", copies: 20 }], [{ definitionId: "IT-001", copies: 40 }]);
    toMain(state, "P1");
    state.players.P1.availableCapacity = 20;
...
test("Chief Marketing Officer deals 1 Reputation when the third Marketing Action is played", () => {
    const state = mixedMatch([{ definitionId: "MKT-001", copies: 20 }, { definitionId: "MKT-007", copies: 5 }, { definitionId: "MKT-009", copies: 15 }], [{ definitionId: "IT-001", copies: 40 }]);
    toMain(state, "P1");
...
test("Full Production buffs the whole Production board and grants Breakthrough at four Employees", () => {
    const state = mixedMatch([{ definitionId: "PRD-001", copies: 30 }, { definitionId: "PRD-009", copies: 10 }], [{ definitionId: "IT-001", copies: 40 }]);
    toMain(state, "P1");
    state.players.P1.availableCapacity = 30;
    const workers = [];
...
test("Conveyor Belt buff survives the opponent turn and expires at the start of the next own turn", () => {
    const state = mixedMatch([{ definitionId: "PRD-014", copies: 10 }, { definitionId: "PRD-001", copies: 30 }], [{ definitionId: "IT-001", copies: 40 }]);
    toMain(state, "P1");
    state.players.P1.availableCapacity = 20;
...
test("Presentation restores Reputation when its target is a Lead", () => {
    const state = mixedMatch([{ definitionId: "N-001", copies: 15 }, { definitionId: "N-004", copies: 10 }, { definitionId: "N-005", copies: 15 }], [{ definitionId: "IT-001", copies: 40 }]);
    toMain(state, "P1");
    state.players.P1.availableCapacity = 30;
    state.players.P1.reputation = 10;
