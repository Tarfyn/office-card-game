...
function endedRoom(persistence) {
    let tokenNo = 0;
...
test("v4.4 replay access rejects profiles that were not seated in the room", () => {
    const { rooms, host } = endedRoom();
    assert.throws(() => rooms.getReplayForProfile(host.roomId, "profile-outsider"), (error) => error instanceof RoomError && error.code === "PROFILE_NOT_IN_ROOM");
});
test("v4.4 replay is unavailable before the match has ended", () => {
    ...
    assert.throws(() => rooms.getReplayForProfile(host.roomId, "profile-live"), (error) => error instanceof RoomError && error.code === "REPLAY_NOT_AVAILABLE");
});
