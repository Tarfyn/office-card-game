...
function deck(definitionId) {
    return [{ definitionId, copies: 40 }];
}
function readyMatch(firstPlayerId = "P1") {
...
function forceToHand(state, playerId, definitionId) {
    ...
    if (index < 0)
        throw new Error(`No ${definitionId} available to force into hand`);
...
function mixedMatch(p1Deck, p2Deck) {
    const state = createMatch({
        matchId: "mixed",
        seed: 987,
        firstPlayerId: "P1",
...
test("Actions validate targets, modify Power, resolve, and Archive", () => {
    const state = mixedMatch([{ definitionId: "CS-001", copies: 30 }, { definitionId: "N-006", copies: 10 }], [{ definitionId: "IT-001", copies: 40 }]);
    toMain(state, "P1");
...
test("temporary Power modifiers expire after the current turn", () => {
    const state = mixedMatch([{ definitionId: "CS-001", copies: 30 }, { definitionId: "N-007", copies: 10 }], [{ definitionId: "IT-001", copies: 40 }]);
    toMain(state, "P1");
...
test("Systems occupy Support slots and activated abilities can gain Capacity above max", () => {
    const state = mixedMatch([{ definitionId: "IT-001", copies: 30 }, { definitionId: "IT-014", copies: 10 }], [{ definitionId: "CS-001", copies: 40 }]);
    toMain(state, "P1");
...
test("System activated ability checks its board condition", () => {
    const state = mixedMatch([{ definitionId: "IT-014", copies: 20 }, { definitionId: "CS-001", copies: 20 }], [{ definitionId: "CS-001", copies: 40 }]);
    toMain(state, "P1");
...
test("Assembly Line applies a dynamic continuous Battle Phase Power bonus", () => {
    const state = mixedMatch([{ definitionId: "PRD-001", copies: 30 }, { definitionId: "PRD-013", copies: 10 }], [{ definitionId: "CS-001", copies: 40 }]);
    toMain(state, "P1");
    state.players.P1.availableCapacity = 20;
...
test("Please Hold interrupts an attack through Priority and Chain resolution", () => {
    const state = mixedMatch([{ definitionId: "PRD-001", copies: 20 }, { definitionId: "CS-010", copies: 20 }], [{ definitionId: "IT-001", copies: 40 }]);
    toMain(state, "P1");
...
test("Works on My Machine negates a targeted Action Chain Item", () => {
    const state = mixedMatch([{ definitionId: "IT-001", copies: 20 }, { definitionId: "IT-013", copies: 20 }], [{ definitionId: "CS-001", copies: 20 }, { definitionId: "N-006", copies: 20 }]);
    toMain(state, "P1");
...
test("an interactive Action resolves after two consecutive passes when there is no response", () => {
    const state = mixedMatch([{ definitionId: "IT-001", copies: 40 }], [{ definitionId: "CS-001", copies: 20 }, { definitionId: "N-006", copies: 20 }]);
    toMain(state, "P1");
    const itEmployee = forceToHand(state, "P1", "IT-001");
    playEmployee(state, "P1", itEmployee, 0);
    finishTurn(state, "P1");
...
function forceInstanceToArchive(state, instanceId) {
    ...
    if (card.zone === "EMPLOYEE_FIELD" && card.slot !== null)
        player.employeeField[card.slot] = null;
    if (card.zone === "SUPPORT_FIELD" && card.slot !== null)
        player.supportField[card.slot] = null;
    card.zone = "ARCHIVE";
    card.slot = null;
    card.objectVersion += 1;
    card.faceUp = true;
...
function forceToArchive(state, playerId, definitionId) {
...
test("Ticket Reopened returns a CS Staff Employee and Archives the same object at the end of the next own turn", () => {
    const state = mixedMatch([{ definitionId: "CS-001", copies: 25 }, { definitionId: "CS-008", copies: 15 }], [{ definitionId: "IT-001", copies: 40 }]);
    toMain(state, "P1");
    state.players.P1.availableCapacity = 10;
...
test("Friday Deployment plays an IT System for free and only penalizes Reputation if that deployed object leaves", () => {
    const make = () => mixedMatch([{ definitionId: "IT-001", copies: 15 }, { definitionId: "IT-011", copies: 10 }, { definitionId: "IT-014", copies: 15 }], [{ definitionId: "CS-001", copies: 40 }]);
    const safe = make();
    toMain(safe, "P1");
...
test("Overtime grants a second attack and Archives the worker in End Phase after two attacks", () => {
    const state = mixedMatch([{ definitionId: "PRD-001", copies: 25 }, { definitionId: "PRD-011", copies: 15 }], [{ definitionId: "CS-001", copies: 40 }]);
    toMain(state, "P1");
...
test("Promotion Archives materials without destroying them and plays Plant Manager in Onboarding", () => {
    const state = mixedMatch([{ definitionId: "PRD-001", copies: 30 }, { definitionId: "PRD-008", copies: 10 }], [{ definitionId: "CS-001", copies: 40 }]);
    toMain(state, "P1");
    state.players.P1.availableCapacity = 20;
...
test("Approval Required can make the opponent pay 1 Capacity and then lets the Action resolve", () => {
    const state = mixedMatch([{ definitionId: "OFC-007", copies: 20 }, { definitionId: "CS-001", copies: 20 }], [{ definitionId: "IT-001", copies: 20 }, { definitionId: "N-007", copies: 20 }]);
    toMain(state, "P1");
...
test("System Administrator reduces only the first IT System cost each turn", () => {
    const state = mixedMatch([{ definitionId: "IT-003", copies: 10 }, { definitionId: "IT-014", copies: 30 }], [{ definitionId: "CS-001", copies: 40 }]);
    toMain(state, "P1");
    state.players.P1.availableCapacity = 20;
...
test("Process Manager reduces the first Process Incident Set Cost", () => {
    const state = mixedMatch([{ definitionId: "OFC-004", copies: 10 }, { definitionId: "OFC-007", copies: 30 }], [{ definitionId: "CS-001", copies: 40 }]);
    toMain(state, "P1");
    state.players.P1.availableCapacity = 20;
...
test("Call Dodged redirects an attack to another legal Employee", () => {
    const state = mixedMatch([{ definitionId: "CS-001", copies: 30 }, { definitionId: "CS-011", copies: 10 }], [{ definitionId: "IT-001", copies: 40 }]);
    toMain(state, "P1");
    state.players.P1.availableCapacity = 20;
...
test("Wrong Department retargets a Chain Item but preserves original target legality", () => {
    const state = mixedMatch([{ definitionId: "CS-001", copies: 30 }, { definitionId: "CS-012", copies: 10 }], [{ definitionId: "IT-001", copies: 20 }, { definitionId: "N-006", copies: 20 }]);
    toMain(state, "P1");
    state.players.P1.availableCapacity = 20;
    const first = forceToHand(state, "P1", "CS-001");
    playEmployee(state, "P1", first, 0);
...
test("Customer Advocate can respond from the field and redirect an attack to itself", () => {
    const state = mixedMatch([{ definitionId: "CS-001", copies: 20 }, { definitionId: "CS-005", copies: 20 }], [{ definitionId: "IT-001", copies: 40 }]);
    toMain(state, "P1");
    state.players.P1.availableCapacity = 20;
...
test("Ticket Specialist queues a draw trigger when a Ticket returns another Employee from Archive", () => {
    const state = mixedMatch([{ definitionId: "CS-003", copies: 10 }, { definitionId: "CS-001", copies: 15 }, { definitionId: "CS-008", copies: 15 }], [{ definitionId: "IT-001", copies: 40 }]);
    toMain(state, "P1");
    state.players.P1.availableCapacity = 20;
...
test("a Ticket Specialist revived by Ticket Reopened does not trigger from its own return", () => {
    const state = mixedMatch([{ definitionId: "CS-003", copies: 20 }, { definitionId: "CS-008", copies: 20 }], [{ definitionId: "IT-001", copies: 40 }]);
    toMain(state, "P1");
    state.players.P1.availableCapacity = 20;
...
test("Performance Marketer reacts to Marketing Reputation loss through the Trigger Queue", () => {
    const state = mixedMatch([{ definitionId: "MKT-003", copies: 20 }, { definitionId: "MKT-012", copies: 20 }], [{ definitionId: "CS-001", copies: 40 }]);
    toMain(state, "P1");
    state.players.P1.availableCapacity = 20;
...
test("Plant Manager uses Battle Event excess Power as a once-per-turn Breakthrough-style trigger", () => {
    const state = mixedMatch([{ definitionId: "PRD-001", copies: 15 }, { definitionId: "PRD-004", copies: 15 }, { definitionId: "PRD-008", copies: 10 }], [{ definitionId: "CS-001", copies: 40 }]);
    toMain(state, "P1");
...
test("Campaign Manager reduces the first Campaign Action cost each turn", () => {
    const state = mixedMatch([{ definitionId: "MKT-005", copies: 20 }, { definitionId: "MKT-012", copies: 20 }], [{ definitionId: "CS-001", copies: 40 }]);
    toMain(state, "P1");
    state.players.P1.availableCapacity = 20;
...
test("intrinsic Breakthrough converts battle excess Power into Reputation damage", () => {
    const definitions = {
        ...alphaDefinitions,
        "TEST-BREAK": {
            id: "TEST-BREAK", version: 1, name: "Breakthrough Tester", cardType: "EMPLOYEE",
            department: "PRODUCTION", rank: "STAFF", cost: { play: 2 }, power: 4, keywords: ["BREAKTHROUGH"]
        }
    };
    const state = createMatch({
...
function forceTopDefinitions(state, playerId, definitionsInDrawOrder) {
    const player = state.players[playerId];
    const chosen = [];
    for (const definitionId of definitionsInDrawOrder) {
        ...
        if (!id) {
            ...
            if (!id)
                throw new Error(`No ${definitionId} available to put on top`);
...
test("battle payload reports both destroyed Employees on equal Power", () => {
    ...
    const destroyed = battle?.data?.destroyedIds;
...
function customMatch(p1Deck, p2Deck, extra = {}) {
...
test("Emergency Patch prevents a targeted card-effect destruction", () => {
    const state = customMatch([{ definitionId: "TEST-DESTROY", copies: 40 }], [
        { definitionId: "IT-001", copies: 20 },
...
test("viewer projection hides opponent hand and face-down Incident identity", () => {
    const state = mixedMatch([{ definitionId: "CS-001", copies: 40 }], [
        { definitionId: "IT-001", copies: 20 },
        { definitionId: "IT-012", copies: 20 }
    ]);
    toMain(state, "P1");
...
test("Sick Leave replaces battle destruction and prevents Breakthrough", () => {
    ...
    const state = customMatch([{ definitionId: "TEST-BREAKER", copies: 40 }], [
        { definitionId: "CS-001", copies: 20 },
    ...
    const battle = [...state.eventLog].reverse().find((event) => event.type === "BATTLE_RESOLVED");
    assert(Array.isArray(battle?.data?.destroyedIds) && battle.data.destroyedIds.length === 0, "Battle should report no actual destroyed Employee");
});
test("Hotfix protects an IT System until the start of its controller's next turn", () => {
    ...
    ], [{ definitionId: "TEST-DESTROY", copies: 40 }], { "TEST-DESTROY": testDestroyDefinition });
    toMain(state, "P1");
    state.players.P1.availableCapacity = 20;
    const system = forceToHand(state, "P1", "IT-014");
    playSystem(state, "P1", system, 0);
...
test("legal-action projection reflects battle-destruction response priority", () => {
    ...
    const state = customMatch([{ definitionId: "TEST-BREAKER-LEGAL", copies: 40 }], [
        { definitionId: "CS-001", copies: 20 },
        { definitionId: "N-011", copies: 20 }
