...
function startActive(rooms, nowRef) {
    let tokenNo = 0;
...
test("v4.0 response timeout auto-passes priority instead of forfeiting the match", () => {
    ...
    let tokenNo = 0;
    const options = { roomIdFactory: () => "RSP400", tokenFactory: () => `rsp-${++tokenNo}`, seedFactory: () => 42, firstPlayerFactory: () => "P1", nowFactory: () => now.value, timerProfiles: [profile({ turnSeconds: 60, responseSeconds: 3 })], persistence: store.adapter };
...
test("v4.0 timer checkpoints survive restart without charging server downtime and runtime is wired to server/UI", () => {
    ...
    const store = memoryPersistence();
    let tokenNo = 0;
    const base = { tokenFactory: () => `persist-${++tokenNo}`, seedFactory: () => 44, firstPlayerFactory: () => "P1", nowFactory: () => now.value, timerProfiles: [profile({ turnSeconds: 20 })], persistence: store.adapter };
