...
export function analyzeContentGaps() {
    const departments = ['CUSTOMER_SERVICE', 'IT', 'OFFICE', 'MARKETING', 'PRODUCTION'];
    const audits = departments.map((department) => {
        const cards = Object.values(alphaDefinitions).filter((card) => card.department === department);
        const cardTypes = { EMPLOYEE: 0, ACTION: 0, INCIDENT: 0, SYSTEM: 0 };
        const tags = new Map();
        for (const card of cards) {
            cardTypes[card.cardType] += 1;
        ...
        for (const entry of starter?.cards ?? []) {
            ...
            if (def?.cardType !== "EMPLOYEE")
                continue;
            starterEmployees += entry.copies;
            if ((def.cost?.play ?? 0) <= 2)
                starterCheapEmployees += entry.copies;
        }
    ...
    for (const row of audits) {
        if (row.totalCards < 18)
            gaps.push({ department: row.department, kind: "POOL_DEPTH", detail: `${row.totalCards} department cards; below the 18-card audit threshold.` });
        if (row.cardTypes.INCIDENT <= 1)
            gaps.push({ department: row.department, kind: "REACTIVE_DEPTH", detail: `Only ${row.cardTypes.INCIDENT} department Incident definition${row.cardTypes.INCIDENT === 1 ? '' : 's'}.` });
        if (row.cheapEmployeeDefinitions < 3)
            gaps.push({ department: row.department, kind: "EARLY_EMPLOYEE_VARIETY", detail: `Only ${row.cheapEmployeeDefinitions} department Employee definitions cost 2 or less.` });
    }
