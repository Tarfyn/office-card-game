...
function printedCost(def) {
    return def.cost?.play ?? def.cost?.set ?? 0;
}
function definitionId(state, instanceId) {
    return state.cards[instanceId]?.definitionId ?? "";
}
function ownFieldIds(state, playerId) {
    return [...state.players[playerId].employeeField, ...state.players[playerId].supportField].filter((id) => Boolean(id));
}
function countDefinitionOnField(state, playerId, id) {
    return ownFieldIds(state, playerId).filter((instanceId) => definitionId(state, instanceId) === id).length;
}
function countCardTypeOnField(state, playerId, type) {
    return ownFieldIds(state, playerId).filter((instanceId) => state.definitions[definitionId(state, instanceId)]?.cardType === type).length;
}
function countDepartmentEmployees(state, playerId, department) {
...
function cardValue(state, instanceId, perspective) {
    ...
    if (def.cardType === "EMPLOYEE") {
        ...
        if (def.rank === "LEAD")
            value += 1.2;
        if (def.rank === "EXECUTIVE")
            value += 1.8;
    }
    if (department === ownDeckDepartment)
        value += 1.2;
    if (def.tags?.includes("TICKET") && ownDeckDepartment === "CUSTOMER_SERVICE")
        value += 0.8;
    if (def.tags?.includes("SYSTEM") && ownDeckDepartment === "IT")
        value += 0.8;
    if (def.tags?.includes("CAMPAIGN") && ownDeckDepartment === "MARKETING")
        value += 0.8;
    if ((def.tags?.includes("OUTPUT") || def.tags?.includes("SHIFT")) && ownDeckDepartment === "PRODUCTION")
        value += 0.8;
    if ((def.tags?.includes("MEETING") || def.tags?.includes("PROCESS")) && ownDeckDepartment === "OFFICE")
        value += 0.6;
    return value;
}
function inferDeckDepartment(state, playerId) {
...
function chooseTargetIds(state, playerId, selector) {
...
function buildTargets(state, playerId, choices) {
    const targets = {};
    for (const choice of choices)
        targets[choice.selectorId] = chooseTargetIds(state, playerId, choice);
    return targets;
}
function mulliganChoice(state, playerId) {
    ...
    const keep = new Set();
    for (const { instanceId, def } of cheapEmployees.slice(0, department === "PRODUCTION" ? 3 : 2))
        keep.add(instanceId);
    const keepFirst = (wanted) => {
        for (const id of wanted) {
            const found = ids.find((x) => x.id === id && !keep.has(x.instanceId));
            if (found) {
                keep.add(found.instanceId);
                return;
            }
        }
    };
    if (department === "CUSTOMER_SERVICE") {
        ...
        for (const { instanceId, def } of ids) {
            if (def.cardType === "EMPLOYEE" && printedCost(def) <= 2 && keep.size < 4)
                keep.add(instanceId);
        }
    ...
    return hand.filter((instanceId) => {
        if (keep.has(instanceId))
            return false;
        const def = state.definitions[definitionId(state, instanceId)];
        if (def.cardType === "EMPLOYEE" && printedCost(def) >= 5)
            return true;
        if (department === "PRODUCTION" && def.cardType !== "EMPLOYEE" && cheapEmployees.length < 2)
            return true;
        if (def.cardType === "ACTION" && printedCost(def) >= 4)
            return true;
        return false;
    });
}
function attackThreatScore(state, playerId) {
    const pending = state.pendingAttack;
    if (!pending)
        return 0;
    if (pending.targetId === null)
        return 12 + getCurrentPower(state, pending.attackerId);
    ...
    if (attackerPower > targetPower)
        return 8 + targetValue;
    if (attackerPower === targetPower)
        return 4 + targetValue * 0.5;
    return -3;
}
function buildResponseTargets(state, playerId, sourceId, choices) {
...
function chooseResponse(state, playerId, legal) {
    if (legal.responseOptions.length === 0)
...
function chooseAttack(state, playerId, legal) {
    const options = [];
    const department = inferDeckDepartment(state, playerId);
    for (const attack of legal.attacks) {
        ...
        for (const targetId of attack.targetIds) {
            ...
            if (attackerPower > defenderPower)
                score = 60 + (attackerPower - defenderPower) + defenderValue;
            else if (attackerPower === defenderPower)
                score = 30 + defenderValue * 0.4;
            else
                score = -20 - (defenderPower - attackerPower);
            // Archetype combat posture: reactive/control decks preserve engines, Production accepts trades.
            if (attackerPower === defenderPower && department === "CUSTOMER_SERVICE")
                score = 8 + defenderValue - attackerValue * 1.15;
            if (attackerPower === defenderPower && department === "OFFICE")
                score = 14 + defenderValue - attackerValue * 0.75;
            if (attackerPower === defenderPower && department === "PRODUCTION")
                score += 12;
            if (department === "CUSTOMER_SERVICE" && attackerValue >= 10 && attackerPower <= defenderPower)
                score -= 12;
            options.push({ attackerId: attack.attackerId, targetId, score });
        }
    }
...
function chooseMainPlay(state, playerId, legal) {
    const department = inferDeckDepartment(state, playerId);
    const candidates = [];
    ...
    for (const option of legal.playableEmployees) {
        ...
        if (department === "PRODUCTION")
            score += 3.5 + Math.max(0, 3 - ownEmployees) * 1.5;
        if (department === "CUSTOMER_SERVICE" && ["CS-003", "CS-005"].includes(def.id))
            score += 2;
        if (department === "IT" && def.id === "IT-003" && ownSystems === 0)
            score += 5;
        if (department === "OFFICE" && ["OFC-003", "OFC-004"].includes(def.id))
            score += 3;
        if (department === "MARKETING" && ["MKT-001", "MKT-005"].includes(def.id))
            score += 3;
        if (def.rank === "EXECUTIVE")
            score -= 1.2;
        ...
        if (bestOption)
            score -= bestOption.promotionMaterialIds.reduce((sum, id) => sum + cardValue(state, id, playerId) * 0.18, 0);
        if (bestOption)
            candidates.push({ score, intent: { type: "PLAY_EMPLOYEE", cardId: option.cardId, slot: bestOption.slot, promotionMaterialIds: bestOption.promotionMaterialIds } });
    }
    for (const option of legal.playableSystems) {
        const def = state.definitions[definitionId(state, option.cardId)];
        let score = cardValue(state, option.cardId, playerId);
        if (department === "IT")
            score += 4 + Math.max(0, 2 - ownSystems);
        if (department === "CUSTOMER_SERVICE" && def.id === "CS-019")
            score += ownIncidents < 2 ? 4 : 1;
        if (department === "CUSTOMER_SERVICE" && def.id === "CS-013")
            score += missingReputation >= 3 ? 3 : 0;
        if (department === "OFFICE" && def.id === "OFC-011")
            score += 3;
        if (department === "MARKETING" && ["MKT-014", "MKT-015"].includes(def.id))
            score += 3;
        if (department === "PRODUCTION" && ["PRD-013", "PRD-014"].includes(def.id))
            score += ownEmployees >= 2 ? 3 : 0;
    ...
    for (const option of legal.settableIncidents) {
        ...
        let score = cardValue(state, option.cardId, playerId);
        if (department === "CUSTOMER_SERVICE")
            score += ownIncidents === 0 ? 5 : ownIncidents === 1 ? 3 : 0.5;
        if (department === "OFFICE")
            score += ownIncidents < 2 ? 2 : 0;
        if (def.id === "CS-010" || def.id === "CS-018")
            score += 1.5;
    ...
    for (const option of legal.playableActions) {
        ...
        if (department === "CUSTOMER_SERVICE") {
            if (def.id === "CS-014")
                score += missingReputation >= 3 ? 7 : missingReputation > 0 ? 2 : -10;
            if (def.id === "CS-008" || def.id === "CS-009")
                score += ownEmployees < 4 ? 5 : 0;
            if (def.id === "CS-016")
                score += countDefinitionOnField(state, playerId, "CS-006") + countDefinitionOnField(state, playerId, "CS-007") === 0 ? 3 : -1;
        }
        if (def.id === "MKT-012") {
        ...
        if (def.id === "PRD-009")
            score += ownEmployees >= 4 ? 10 : ownEmployees === 3 ? 3 : -9;
        if (def.id === "PRD-010")
            score += ownEmployees >= 3 ? 5 : ownEmployees <= 1 ? -7 : 0;
        if (def.id === "OFC-009")
            score += ownEmployees >= 3 ? 6 : ownEmployees <= 1 ? -5 : 0;
    ...
    for (const option of legal.activatableAbilities) {
        ...
        if (def.department === department)
            score += 1;
        if (department === "IT" && def.id === "IT-014")
            score += 4;
...
function choosePendingIntent(state) {
    if (state.phase === "MULLIGAN" && state.status === "SETUP") {
        ...
        if (playerId)
            return { playerId, intent: { type: "MULLIGAN", returnIds: mulliganChoice(state, playerId) } };
    }
...
function nextBotIntent(state) {
    ...
    if (state.phase === "MAIN") {
        ...
        if (play)
            return { playerId, intent: play };
    }
    if (state.phase === "BATTLE") {
        const attack = chooseAttack(state, playerId, legal);
        if (attack)
            return { playerId, intent: attack };
    }
    if (legal.canAdvancePhase)
        return { playerId, intent: { type: "ADVANCE_PHASE" } };
    if (legal.canPassPriority)
        return { playerId, intent: { type: "PASS_PRIORITY" } };
    return null;
}
function eventCount(events, type, playerId) {
    return events.filter((e) => e.type === type && (!playerId || e.playerId === playerId)).length;
}
function reputationDamage(events) {
    let total = 0;
...
function reputationRestored(events, playerId) {
    let total = 0;
    for (const event of events) {
        if (event.type !== "REPUTATION_CHANGED" || event.playerId !== playerId)
            continue;
        const amount = Number(event.data?.amount ?? 0);
        if (amount > 0)
            total += amount;
...
function eventDefinitionIds(state, type, playerId) {
...
function seenDefinitionIds(state, playerId) {
    const ids = new Set();
    for (const id of eventDefinitionIds(state, "CARD_DRAWN", playerId))
        ids.add(id);
    for (const id of eventDefinitionIds(state, "CARD_PLAYED", playerId))
        ids.add(id);
    return [...ids];
}
function runMatch(p1DeckId, p2DeckId, firstPlayer, seed, config) {
    ...
    while (state.status !== "ENDED" && state.turnNumber <= config.maxTurns && steps < config.maxSteps) {
        ...
        if (!execution.response.accepted) {
            ...
            const fallback = legal.canPassPriority ? { type: "PASS_PRIORITY" } : (state.activePlayerId === decision.playerId && legal.canAdvancePhase ? { type: "ADVANCE_PHASE" } : null);
            if (!fallback) {
                stuck = true;
...
function summarizePair(deckA, deckB, games) {
...
function summarizeDeck(deckId, games) {
...
function buildCardBalanceStats(games) {
    const map = new Map();
    for (const game of games) {
        for (const playerId of ["P1", "P2"]) {
            ...
            for (const id of new Set(seen)) {
                const row = map.get(id) ?? { gamesSeen: 0, winsWhenSeen: 0, gamesPlayed: 0, winsWhenPlayed: 0, totalCopiesPlayed: 0 };
                row.gamesSeen += 1;
                if (won)
                    row.winsWhenSeen += 1;
                map.set(id, row);
            ...
            for (const id of playedSet) {
                ...
                row.gamesPlayed += 1;
                if (won)
                    row.winsWhenPlayed += 1;
                row.totalCopiesPlayed += played.filter((value) => value === id).length;
                map.set(id, row);
            }
        }
    }
...
function buildBalanceAnalytics(games, matchups, deckIds) {
    ...
    const endReasons = {};
    for (const game of games)
        endReasons[game.reason] = (endReasons[game.reason] ?? 0) + 1;
    ...
    for (const matchup of matchups) {
        ...
        if (completedPair >= 2 && (matchup.deckAWinRate >= 70 || matchup.deckAWinRate <= 30))
            signals.push({ kind: "MATCHUP_SKEW", label: `${matchup.deckA} vs ${matchup.deckB}`, value: matchup.deckAWinRate, sample: completedPair, sampleLabel: sampleLabel(completedPair) });
    }
    const firstRate = completed.length ? pct(firstWins / completed.length) : 0;
    if (completed.length >= 4 && (firstRate >= 60 || firstRate <= 40))
        signals.push({ kind: "FIRST_PLAYER_SKEW", label: "First-player win rate", value: firstRate, sample: completed.length, sampleLabel: sampleLabel(completed.length) });
    const timeoutRate = games.length ? pct(games.filter((game) => game.timedOut).length / games.length) : 0;
    if (games.length >= 4 && timeoutRate >= 10)
        signals.push({ kind: "TIMEOUT_RATE", label: "Turn/step-cap or bot-stuck rate", value: timeoutRate, sample: games.length, sampleLabel: sampleLabel(games.length) });
    return {
        endReasons,
...
export function runBalanceMatchupSet(plan) {
    if (!plan.matchups.length)
        throw new Error("At least one matchup is required.");
    const games = [];
    let sequence = 0;
    for (const pair of plan.matchups) {
        if (!alphaDeckPresets[pair.deckA])
            throw new Error(`Unknown deck preset: ${pair.deckA}`);
        if (!alphaDeckPresets[pair.deckB])
            throw new Error(`Unknown deck preset: ${pair.deckB}`);
        if (pair.deckA === pair.deckB)
            throw new Error(`Matchup must use two different decks: ${pair.deckA}`);
        for (let game = 0; game < plan.gamesPerMatchup; game += 1) {
            ...
            const firstPlayer = plan.alternateFirstPlayer === false ? "P1" : (Math.floor(game / 2) % 2 === 0 ? "P1" : "P2");
...
export function runBalanceSeries(config) {
    ...
    for (let a = 0; a < deckIds.length; a += 1) {
        for (let b = a + 1; b < deckIds.length; b += 1) {
            for (let game = 0; game < config.gamesPerMatchup; game += 1) {
                ...
                const firstPlayer = ((game + a + b) % 2 === 0) ? "P1" : "P2";
    ...
    for (let a = 0; a < deckIds.length; a += 1)
        for (let b = a + 1; b < deckIds.length; b += 1)
            matchups.push(summarizePair(deckIds[a], deckIds[b], games));
...
function parseMatchupArg(raw) {
    ...
    const pairs = raw.split(",").map((entry) => entry.trim()).filter(Boolean).map((entry) => {
        ...
        if (!deckA || !deckB)
            throw new Error(`Invalid --matchups entry: ${entry}. Use deck-a:deck-b.`);
        return { deckA, deckB };
    });
...
function main() {
    ...
    for (const deck of report.decks)
        console.log(`  ${deck.deckId}: ${deck.wins}-${deck.losses} · ${deck.winRate}% · timeout ${deck.timeouts} · avg ${deck.averageTurns} turns · actions ${deck.averageActionsResolved} · incidents ${deck.averageIncidentsActivated} · attacks ${deck.averageAttacksDeclared}`);
