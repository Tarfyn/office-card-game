...
export function normalizePlaytestFilter(filter = {}) {
...
export function filterPlaytestRecords(records, filter = {}) {
    const normalized = normalizePlaytestFilter(filter);
    let filtered = records.filter((record) => {
        if (normalized.mode && record.mode !== normalized.mode)
            return false;
        if (normalized.department && !([record.seats.P1, record.seats.P2].some((seat) => seat?.department === normalized.department)))
            return false;
        if (normalized.deckId && !([record.seats.P1, record.seats.P2].some((seat) => seat?.deckId === normalized.deckId)))
            return false;
...
export function playtestAnalyticsDimensions(records) {
    const departments = new Set();
    const decks = new Map();
    const endedAt = [];
    for (const record of records) {
        if (record.status === "ENDED" && record.endedAt != null && Number.isFinite(record.endedAt))
            endedAt.push(record.endedAt);
        for (const playerId of ["P1", "P2"]) {
            const seat = record.seats[playerId];
            if (!seat)
                continue;
            departments.add(seat.department);
            if (!decks.has(seat.deckId))
                decks.set(seat.deckId, { deckId: seat.deckId, deckName: seat.deckName, department: seat.department });
        }
    }
    return {
...
function bucket(records) {
    ...
    const durations = ended.map((record) => record.telemetry.matchElapsedSeconds).filter((value) => value != null && Number.isFinite(value));
    return {
...
export function aggregatePlaytestAnalytics(records, now = Date.now()) {
    ...
    const durations = completed.map((record) => record.telemetry.matchElapsedSeconds).filter((value) => value != null && Number.isFinite(value));
    ...
    for (const record of completed) {
        ...
        for (const playerId of ["P1", "P2"]) {
            ...
            if (seat) {
                ...
                deck.appearances += 1;
                if (record.firstPlayerId === playerId) {
                    deck.firstAppearances += 1;
                    if (record.winnerId === playerId)
                        deck.firstWins += 1;
                }
                else {
                    deck.secondAppearances += 1;
                    if (record.winnerId === playerId)
                        deck.secondWins += 1;
                }
                if (!record.winnerId)
            ...
            for (const kind of ["TURN", "RESPONSE", "DECISION"]) {
        ...
        maxTurnSeconds = Math.max(maxTurnSeconds, turnMax);
        maxResponseSeconds = Math.max(maxResponseSeconds, responseMax);
        if (turnMax >= frictionThresholds.longTurnSeconds)
            matchesWithLongTurn += 1;
        if (responseMax >= frictionThresholds.longResponseSeconds)
            matchesWithLongResponse += 1;
        for (const activity of record.cardActivity ?? []) {
            ...
            if (activity.observedInstances > 0)
                value.observedSeatMatches += 1;
            if (activity.playedInstances > 0) {
                value.playedSeatMatches += 1;
                if (!record.winnerId)
                    value.drawsWhenPlayed += 1;
                else if (record.winnerId === activity.playerId)
                    value.winsWhenPlayed += 1;
                else
                    value.lossesWhenPlayed += 1;
            }
            value.observedInstances += Number(activity.observedInstances || 0);
            value.playedInstances += Number(activity.playedInstances || 0);
            value.draws += Number(activity.draws || 0);
            value.plays += Number(activity.plays || 0);
            value.activations += Number(activity.activations || 0);
            value.attacks += Number(activity.attacks || 0);
            cards.set(activity.definitionId, value);
        }
        if (matchDisconnects > 0)
            matchesWithDisconnects += 1;
    }
    const modeRecords = (mode) => completed.filter((record) => record.mode === mode);
    return {
        generatedAt: now,
        totals: {
...
export function playtestRecordsCsv(records) {
...
export function playtestCardActivityCsv(cards) {
