...
export type Department = "CUSTOMER_SERVICE" | "IT" | "OFFICE" | "MARKETING" | "PRODUCTION" | "NEUTRAL";
...
export type Trigger = {
    ...
    player: RelativePlayer;
    causedByDepartment?: Department;
} | {
    event: "BATTLE_EMPLOYEE_DESTROYED";
    attackerController?: RelativePlayer;
    attackerFilter?: CardFilter;
    destroyedController?: RelativePlayer;
    destroyedFilter?: CardFilter;
} | {
    event: "CARD_ARCHIVED";
    controller: RelativePlayer;
    cardFilter?: CardFilter;
    sourceOnly?: boolean;
    causedByTag?: string;
    causedByController?: RelativePlayer;
} | {
    event: "ACTION_WOULD_BE_ARCHIVED";
    controller: RelativePlayer;
    cardFilter?: CardFilter;
} | {
    event: "CARD_DELAYED";
    ...
    event: "BATTLE_DESTRUCTION_PENDING";
    controller: RelativePlayer;
    cardFilter?: CardFilter;
} | {
...
export type Condition = {
    ...
    type: "CARDS_PLAYED_BY_TAG_EQUALS";
    controller: RelativePlayer;
    tag: string;
    value: number;
} | {
    type: "CARDS_PLAYED_BY_TAG_AT_LEAST";
    controller: RelativePlayer;
    tag: string;
    value: number;
} | {
    type: "ACTIONS_PLAYED_TOTAL_EQUALS";
    controller: RelativePlayer;
    value: number;
} | {
    type: "ACTIVE_PLAYER_IS";
    player: RelativePlayer;
} | {
    type: "TARGET_MATCHES_FILTER";
    target: string;
    filter: CardFilter;
} | {
    type: "SOURCE_PRINTED_COST_AT_LEAST";
    amount: number;
} | {
    type: "HAS_AVAILABLE_CAPACITY";
    player: RelativePlayer;
    amount: number;
} | {
    type: "HAS_FREE_SLOT";
    ...
    type: "TARGET_IS_SAME_OBJECT_IN_ZONE";
    target: string;
    zone: Zone;
} | {
    type: "TARGET_NOT_SAME_OBJECT_IN_ZONE";
    target: string;
    zone: Zone;
} | {
    type: "TARGET_ATTACKS_USED_AT_LEAST";
    target: string;
    amount: number;
} | {
    type: "EVENT_BATTLE_EXCESS_POWER_AT_LEAST";
    amount: number;
} | {
    type: "EVENT_BREAKTHROUGH_NOT_APPLIED";
} | {
    type: "CHAIN_ITEM_HAS_EFFECT";
    effectType: "DESTROY_TARGET";
} | {
    type: "TARGET_IS_TRIGGERING_CHAIN_TARGET";
    target: string;
} | {
...
export type Effect = {
    ...
    type: "LOSE_AVAILABLE_CAPACITY";
    player: RelativePlayer;
    amount: number;
} | {
    type: "LOSE_REPUTATION";
    player: RelativePlayer;
    amount: number | {
        fromEvent: "BATTLE_EXCESS_POWER";
    };
} | {
    type: "RESTORE_REPUTATION";
    player: RelativePlayer;
    amount: number;
} | {
    type: "MODIFY_POWER";
    ...
    type: "PREVENT_ATTACK";
    target: string;
    duration: "END_OF_TURN" | "THROUGH_NEXT_CONTROLLER_BATTLE_PHASE";
} | {
    type: "FOR_EACH_MATCHING";
    selector: TargetSelector;
    effects: Effect[];
} | {
    type: "RESET_TARGET_TEMPORARY";
    target: string;
} | {
    type: "DESTROY_TARGET";
    target: string;
    ...
    type: "ADD_DESTRUCTION_SHIELD";
    target: string;
    cause: "CARD_EFFECT";
    duration: "END_OF_TURN" | "UNTIL_CHAIN_ITEM_RESOLVES" | "UNTIL_START_OF_NEXT_OWN_TURN";
    onlyOpponentSource?: boolean;
} | {
    type: "ADD_DESTRUCTION_SHIELD_TO_TRIGGERING_TARGETS";
    cause: "CARD_EFFECT";
    duration: "UNTIL_CHAIN_ITEM_RESOLVES";
    filter?: CardFilter;
} | {
    type: "MOVE_TARGET";
    target: string;
    to: "ARCHIVE" | "HAND" | "EMPLOYEE_FIELD" | "DECK";
    firstFreeSlot?: boolean;
    playMethod?: PlayMethod;
    shuffleAfter?: boolean;
} | {
    type: "PLAY_TARGET";
    target: string;
    ignoreCapacityCost?: boolean;
    costAdjustment?: number;
    playMethod: PlayMethod;
    firstFreeSupportSlot?: boolean;
    firstFreeEmployeeSlot?: boolean;
} | {
    type: "NEGATE_CHAIN_ITEM";
    target: "TRIGGERING_CHAIN_ITEM";
} | {
    type: "REDIRECT_ATTACK_TARGET";
    newTarget: string | "SOURCE";
} | {
    type: "REDIRECT_CHAIN_TARGET";
    target: "TRIGGERING_CHAIN_ITEM";
    targetKey: string;
    newTarget: string | "SOURCE";
} | {
    type: "DELAY_CHAIN_ITEM";
    target: "TRIGGERING_CHAIN_ITEM";
    until: "START_OF_NEXT_CONTROLLER_MAIN_PHASE";
} | {
    type: "END_CURRENT_ATTACK";
} | {
    type: "ARCHIVE_FROM_HAND_SELECT";
    player: RelativePlayer;
    min: number;
    max: number;
} | {
    type: "RESTRICT_PLAY_TARGET";
    target: string;
    duration: "END_OF_TURN";
} | {
    type: "MODIFY_PROMOTION_VALUE";
    target: string;
    amount: number;
    duration: "END_OF_TURN";
} | {
    type: "ADD_NEXT_PROMOTION_REDUCTION";
    player: RelativePlayer;
    department: Department;
    amount: number;
    ...
    type: "MOVE_TRIGGERING_CHAIN_SOURCE";
    to: "HAND";
    restrictPlayForTurn?: boolean;
} | {
    type: "REVEAL_RANDOM_HAND_CARD";
    player: RelativePlayer;
    storeAs: string;
    to: "BOTH" | "CONTROLLER";
} | {
    type: "REVEAL_FACE_DOWN_SUPPORT";
    player: RelativePlayer;
    to: RelativePlayer;
    duration: "END_OF_TURN";
} | {
    type: "PREVENT_ATTACK_EXCEPT";
    targets: string;
    except: string;
    duration: "END_OF_TURN";
} | {
    ...
    type: "OFFER_CHOICE";
    player: RelativePlayer | "TRIGGERING_PLAYER";
...
export type Ability = {
    ...
    effects: Array<{
        type: "MODIFY_POWER" | "MODIFY_COST" | "SET_ACTION_PLAY_LIMIT";
        amount: number;
    }>;
    phases?: Phase[];
    usageLimit?: UsageLimit;
...
export type EventType = "MATCH_CREATED" | "MULLIGAN_COMPLETED" | "TURN_STARTED" | "DRAW_SKIPPED" | "CARD_DRAWN" | "PHASE_CHANGED" | "CAPACITY_CHANGED" | "CARD_PLAYED" | "PROMOTION_COMPLETED" | "INCIDENT_SET" | "INCIDENT_ACTIVATED" | "CHAIN_ITEM_ADDED" | "CHAIN_ITEM_NEGATED" | "CHAIN_ITEM_DELAYED" | "CHAIN_RESOLVED" | "PRIORITY_PASSED" | "CHOICE_REQUIRED" | "CHOICE_RESOLVED" | "SCHEDULED_EFFECT_CREATED" | "SCHEDULED_EFFECT_RESOLVED" | "PENDING_EFFECT_RESOLVED" | "ATTACK_ENDED" | "CARD_MOVED" | "ACTION_RESOLVED" | "ABILITY_ACTIVATED" | "POWER_MODIFIED" | "KEYWORD_GRANTED" | "CARD_RESET" | "DESTRUCTION_ATTEMPTED" | "DESTRUCTION_PREVENTED" | "CARD_DESTROYED" | "COST_CALCULATED" | "ATTACKS_MODIFIED" | "ATTACK_DECLARED" | "EMPLOYEE_DESTROYED" | "BATTLE_DESTRUCTION_PENDING" | "BATTLE_RESOLVED" | "BREAKTHROUGH_DAMAGE" | "TRIGGER_QUEUED" | "TRIGGER_CHAIN_CREATED" | "ATTACK_TARGET_REDIRECTED" | "CHAIN_TARGET_REDIRECTED" | "REPUTATION_CHANGED" | "CARD_ARCHIVED" | "ACTION_ARCHIVE_PENDING" | "REPUTATION_LOSS_REDUCED" | "DIRECT_DAMAGE_RIDER" | "REVEAL_PERMISSION_GRANTED" | "DECK_SELECTION_REQUIRED" | "DECK_SELECTION_RESOLVED" | "TRIGGER_TARGET_SELECTION_REQUIRED" | "TRIGGER_TARGET_SELECTION_RESOLVED" | "HAND_SELECTION_REQUIRED" | "HAND_SELECTION_RESOLVED" | "CARD_PLAY_RESTRICTED" | "CARD_DELAYED" | "DECK_SHUFFLED" | "CARD_REVEALED" | "TOP_CARDS_VIEWED" | "GAME_ENDED";
...
export type MatchIntent = {
    ...
    type: "RESOLVE_DECK_SELECTION";
    selectionId: string;
    selectedIds: string[];
    orderedUnselectedIds?: string[];
} | {
    type: "RESOLVE_TRIGGER_TARGET_SELECTION";
    selectionId: string;
    targets: Record<string, string[]>;
} | {
    type: "RESOLVE_HAND_SELECTION";
    selectionId: string;
    selectedIds: string[];
} | {
    type: "RESIGN";
...
export interface MatchCommandResponse {
    ...
    lastEventSeq: number;
    error?: {
        code: "STALE_STATE" | "RULES_ERROR" | "MATCH_MISMATCH" | "INTERNAL_ERROR";
        message: string;
    };
    events: ClientEvent[];
