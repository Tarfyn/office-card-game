// Regression compatibility marker for v7.26 viewer-visible provenance: sourceRef === contribution.sourceInstanceId
import type {
  CardInstance,
  ClientCardView,
  ClientEvent,
...
function isPublicCard(card: CardInstance): boolean {
  if (card.zone === "ARCHIVE" || card.zone === "EMPLOYEE_FIELD" || card.zone === "PENDING") return true;
  if (card.zone === "SUPPORT_FIELD" && card.faceUp) return true;
  return false;
}

function hiddenSupportId(card: CardInstance): string {
  return `hidden-support:${card.controllerId}:${card.slot ?? -1}:${card.objectVersion}`;
}

function hasSupportRevealPermission(state: GameState, viewerId: PlayerId, controllerId: PlayerId): boolean {
...
export function clientCardRef(state: GameState, viewerId: PlayerId, instanceId: string): string | null {
  ...
  if (viewerOwnsKnowledge || isPublicCard(card) || temporarilyRevealed) return instanceId;
  if (card.zone === "SUPPORT_FIELD" && !card.faceUp) return hiddenSupportId(card);
  return null;
}

function projectLegalActions(state: GameState, viewerId: PlayerId, legal: ClientLegalActions): ClientLegalActions {
  const mapCandidates = (ids: string[]) => ids.map((id) => clientCardRef(state, viewerId, id)).filter((id): id is string => id !== null);
  const mapTargetChoices = (choices: ClientLegalActions["playableActions"][number]["targetChoices"]) => choices.map((choice) => ({
    ...choice,
  ...
  return {
    ...
    attacks: legal.attacks.map((attack) => ({ ...attack, targetIds: attack.targetIds.map((id) => id === null ? null : clientCardRef(state, viewerId, id)).filter((id): id is string | null => id !== null || attack.targetIds.includes(null)) }))
  };
}

function projectedSourceIdentity(state: GameState, viewerId: PlayerId, sourceInstanceId?: string): { sourceId?: string; sourceDefinitionId?: string } {
  if (!sourceInstanceId) return {};
  const sourceRef = clientCardRef(state, viewerId, sourceInstanceId);
  if (sourceRef !== sourceInstanceId) return {};
  const source = state.cards[sourceInstanceId];
  return source ? { sourceId: sourceRef, sourceDefinitionId: source.definitionId } : {};
}

function projectLiveStatuses(state: GameState, viewerId: PlayerId, instanceId: string): NonNullable<ClientCardView["liveStatuses"]> {
  const card = state.cards[instanceId];
  const statuses: NonNullable<ClientCardView["liveStatuses"]> = [];
  if (card.onboarding) statuses.push({ kind:"ONBOARDING", label:"Onboarding", detail:"Cannot attack this turn." });
  if (card.attacksUsed > 0) statuses.push({ kind:"ATTACK_USED", label:"Attack used", detail:`${card.attacksUsed} / ${card.maxAttacks} attacks used.` });
  ...
  if (restrictionActive) statuses.push({
  ...
  if (card.maxAttacks !== 1) statuses.push({ kind:"ATTACK_LIMIT", label:`Max attacks ${card.maxAttacks}`, detail:"Current attack allowance differs from the normal 1." });
  for (const modifier of card.keywordModifiers) statuses.push({
  ...
  for (const shield of card.destructionShields) statuses.push({
...
function projectPowerBreakdown(state: GameState, viewerId: PlayerId, instanceId: string): NonNullable<ClientCardView["powerBreakdown"]> {
...
function projectCard(state: GameState, viewerId: PlayerId, instanceId: string): ClientCardView {
  const card = state.cards[instanceId];
  const viewerOwnsKnowledge = card.ownerId === viewerId || card.controllerId === viewerId;
  const temporarilyRevealed = card.zone === "SUPPORT_FIELD" && !card.faceUp && hasSupportRevealPermission(state, viewerId, card.controllerId);
...
function projectPlayer(state: GameState, viewerId: PlayerId, playerId: PlayerId): ClientPlayerView {
...
function projectChain(state: GameState, viewerId: PlayerId): ClientGameState["chain"] {
...
function projectPendingResolutions(state: GameState, viewerId: PlayerId): ClientGameState["pendingResolutions"] {
...
function projectScheduledEffects(state: GameState, viewerId: PlayerId): ClientGameState["scheduledEffects"] {
...
function projectPendingAttack(state: GameState, viewerId: PlayerId): ClientGameState["pendingAttack"] {
  ...
  if (pending.targetId !== null && !targetId) return null;
...
function redactEvent(state: GameState, viewerId: PlayerId, event: GameEvent): ClientEvent | null {
  const out: ClientEvent = structuredClone(event);
  ...
  if (event.type === "CARD_REVEALED" && out.data?.to === "CONTROLLER" && !isOwnerEvent) return null;
  ...
  if (event.type === "CARD_MOVED" && !isOwnerEvent) {
    ...
    if (to === "HAND" || to === "DECK") delete out.cardInstanceId;
    if (to === "SUPPORT_FIELD") {
      const id = event.cardInstanceId;
      const card = id ? state.cards[id] : undefined;
      if (card && !card.faceUp) delete out.cardInstanceId;
    }
  }
  return out;
}

export function projectEventsSince(state: GameState, viewerId: PlayerId, afterSeq = 0): ClientEvent[] {
